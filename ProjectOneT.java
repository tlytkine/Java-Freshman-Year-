...
return (x*y)/(GCD(x,y));  // calls Greatest Common Divisor method to calculate Least Common Multiple 
 }  
public static void AddSubtract(int x,int y) {
int finDenominator = LCM(Fractions[1][i],Fractions[1][i+1]); //finds LCM of two denominators
// First Fraction
...
//Second Fraction
if (Fractions[1][i+1] != tempDenominator) {
...
int[][] Fractions = {{3,1,4,-5,2},{2,-2,3,6,3}}; //2D array 'Fractions' with n columns and 2 rows 
...
int tNumerator = tempNumeratorResult; // temporary variable that is assigned to the numerator result of each operation so the next operation performs the calculation on this numerator
int tDenominator = tempDenominatorResult;// temporary variable that is assigned to the denominator result of each operation so the next operation performs the calculation on this denominator
//Addition 
if (Operators[i] == '+') { 
...
tempNumeratorResult = tempNumeratorOne - tempNumeratorTwo; // subtracts first numerator from second numerator
tempDenominatorResult = tempDenominator; // sets LCM of denominators as the common denominator / result 
}
if (Operators[i] == '/'){ //division 
// uses same process as multiplication but multiplies by the reciprocal (denominator / numerator instead of numerator / denominator) 
